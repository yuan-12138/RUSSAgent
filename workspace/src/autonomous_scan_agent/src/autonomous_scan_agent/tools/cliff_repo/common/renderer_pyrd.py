...
class Renderer(object):
    ...
    def render_front_view(self, verts, bg_img_rgb=None, bg_color=(0, 0, 0, 0)):
        ...
        for n in range(num_people):
            # NOTE: use process=False to avoid trimesh "fixing" meshes into empty vertices in some edge cases
            if verts[n] is None or len(verts[n]) == 0 or self.faces is None or len(self.faces) == 0:
                continue
