...
# -*- coding: utf-8 -*-
"""
capture_local_mover.py

功能与 easy_handeye/scripts/calib_local_mover.py 基本一致：
- 读取一个“关节角位姿库 YAML”（由 capture_pose.py 保存的 poses.yaml）
- 在 GUI 里手动：Next -> Plan -> Execute -> Next ... 循环
- 可选每次执行后调用一个 Trigger service（例如 take_sample）

默认读取：
  ~/.ros/autonomous_scan_agent/capture_poses.yaml

YAML 格式要求（与 capture_pose.py 一致）：
  poses:
    - name: capture_01
      joints:
        iiwa_joint_1: 0.1
...
class LocalMover(QtWidgets.QWidget):
    def __init__(self):
        ...
        rospy.init_node("capture_local_mover", anonymous=True)

        # ---- 参数（与 calib_local_mover.py 对齐）----
        ...
        # 可选：每次执行后触发采样
        self.sample_srv = rospy.get_param("~sample_service", "")
        self.auto_sample = bool(rospy.get_param("~auto_sample", False))
        self.settle_sec = float(rospy.get_param("~settle_sec", 0.5))
        ...
        if self.group_name not in group_names:
            rospy.logwarn("规划组 '%s' 不存在。可用组：%s", self.group_name, ", ".join(group_names))
            self.group_name = "manipulator" if "manipulator" in group_names else group_names[0]
            rospy.logwarn("已自动切换为组 '%s'", self.group_name)
            self.group = MoveGroupCommander(self.group_name, robot_description=desc_key.lstrip("/"), ns=ns)
    ...
    # ---------- UI ----------
    def _build_ui(self):
        self.setWindowTitle("&Capture Local Mover")
        self.resize(700, 440)
